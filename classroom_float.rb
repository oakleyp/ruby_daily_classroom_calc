...
def averages(grade_hash)
    ...
    result_hash.map{|key, value| result_hash[key] = value.to_f/grade_hash[key].length.to_f}
    result_hash
end
...
def class_average(grade_hash)
    all_averages = []
    averages(grade_hash).map {|key, average| all_averages << average.to_i}
    sum = 0
...
def top_students(grade_hash, number_of_students)
    ...
    puts "Class averages given number of students - (#{number_of_students}):"
    puts all_averages
